...
#define P00_CA_FSIZE(_0, X, _2)                                                                                        \
    size_t P99_PASTE2 (p00_ca_fsize_, X);                                                                              \
    P99_UNUSED (P99_PASTE2 (p00_ca_fsize_, X))
...
#define P00_CA_MANGLE(NAME, ACHECKS, PCHECKS)                                                                          \
    P99_PASTE (p00_ca_, NAME, _p00_achecks, P00_CA_MANGLE_LIST ACHECKS, _p00_pchecks, P00_CA_MANGLE_LIST PCHECKS)

P99_CONSTANT (int, P00_CA_CHECK_BUFFLEN, 512);

#define P00_CA_ACHECK_(P, X)                                                                                           \
    if (P99_LIKELY (X)) {                                                                                              \
        if (P99_UNLIKELY (/* passing in a larger array is ok */                                                        \
                          (P99_PASTE2 (p00_ca_fsize_, X) > P99_PASTE2 (p00_ca_asize_, P))                              \
            )) {                                                                                                       \
            char buf [P00_CA_CHECK_BUFFLEN];                                                                           \
            snprintf (                                                                                                 \
                buf, P00_CA_CHECK_BUFFLEN - 1,                                                                         \
                "%s, call from %s, size of parameter " P99_STRINGIFY (X) " is %zu instead of %zu", p00_proto,          \
                p00_call, P99_PASTE2 (p00_ca_asize_, P), P99_PASTE2 (p00_ca_fsize_, X)                                 \
            );                                                                                                         \
        ...
        char buf [P00_CA_CHECK_BUFFLEN];                                                                               \
        snprintf (                                                                                                     \
            buf, P00_CA_CHECK_BUFFLEN - 1, "%s, call from %s, parameter " P99_STRINGIFY (X) ", is null pointer",       \
            p00_proto, p00_call                                                                                        \
        );                                                                                                             \
...
#define P00_CA_PCHECK_(P, X, I)                                                                                        \
    if (P99_LIKELY (X)) {                                                                                              \
        ...
        if (P99_UNLIKELY ((P99_PASTE2 (p00_ca_lsize_, I) != P99_PASTE2 (p00_ca_psize_, P)))) {                         \
            char buf [P00_CA_CHECK_BUFFLEN];                                                                           \
            snprintf (                                                                                                 \
                buf, P00_CA_CHECK_BUFFLEN - 1,                                                                         \
                "%s, call from %s, sizeof " P99_STRINGIFY (X) "[0] is %zu instead of %zu", p00_proto, p00_call,        \
                P99_PASTE2 (p00_ca_psize_, P), P99_PASTE2 (p00_ca_lsize_, I)                                           \
            );                                                                                                         \
            P99_CONSTRAINT_TRIGGER (ERANGE, buf);                                                                      \
        }                                                                                                              \
    } else {                                                                                                           \
...
#define P00_CA_WRAP_DECLARE(NAME, RET, TYPES, STYPES, VARS, ACHECKS, PCHECKS)                                          \
    ...
    inline RET P00_CA_MANGLE (NAME, ACHECKS, PCHECKS) (                                                                \
        char const      *p00_call,                                                                                     \
        P99_IF_EMPTY     ACHECKS () (P00_CA_ASIZES (P99_NARG ACHECKS, P00_ROBUST ACHECKS), )                           \
            P99_IF_EMPTY PCHECKS () (P00_CA_PSIZES (P99_NARG PCHECKS, P00_ROBUST PCHECKS), ) P00_ROBUST TYPES          \
    ) {                                                                                                                \
        char const *const p00_proto = P99_STRINGIFY (NAME) STYPES;                                                     \
        P99_UNUSED (p00_proto);                                                                                        \
        P99_IF_EMPTY ACHECKS () (P00_CA_FSIZES (P99_NARG VARS, P00_ROBUST VARS); {                                     \
        ...
        P99_IF_EQ_1 (P99_IS_TOK (void, RET)) () (return) NAME VARS;                                                    \
    }                                                                                                                  \
...
#define P00_CA_WRAP_DECLARE2(NAME, RET, TYPES, STYPES, VARS, ACHECKS, PCHECKS)                                         \
    P00_CA_WRAP_DECLARE (                                                                                              \
        NAME, RET, TYPES, STYPES, VARS, P99_IF_EMPTY ACHECKS (()) (ACHECKS), P99_IF_EMPTY PCHECKS (()) (PCHECKS)       \
    )
...
#define P00_CA_WRAP_DEFINE(NAME, RET, TYPES, VARS, ACHECKS, PCHECKS)                                                   \
    P00_INSTANTIATE (                                                                                                  \
        RET, P00_CA_MANGLE (NAME, ACHECKS, PCHECKS), char const *p00_call,                                             \
        P99_IF_EMPTY     ACHECKS () (P00_CA_ASIZES (P99_NARG ACHECKS, P00_ROBUST ACHECKS), )                           \
            P99_IF_EMPTY PCHECKS () (P00_CA_PSIZES (P99_NARG PCHECKS, P00_ROBUST PCHECKS), ) P00_ROBUST TYPES          \
    )
...
#define P00_CA_WRAP_DEFINE2(NAME, RET, TYPES, VARS, ACHECKS, PCHECKS)                                                  \
    P00_CA_WRAP_DEFINE (                                                                                               \
        NAME, RET, TYPES, VARS, P99_IF_EMPTY ACHECKS (()) (ACHECKS), P99_IF_EMPTY PCHECKS (()) (PCHECKS)               \
    )
...
P00_DOCUMENT_PERMITTED_ARGUMENT (P99_CA_CALL, 5)
    #define P99_CA_CALL(NAME, ACHECKS, PCHECKS, ...)                                                                   \
        ...
        (__FILE__ ":" P99_STRINGIFY (__LINE__) ": " P99_STRINGIFY (NAME) "(" #__VA_ARGS__ ")",                         \
