...
struct p99_futex_c11 {
        ...
        #define P99_FUTEX_INITIALIZER(INITIAL)                                                                         \
            { .p99_cnt = (INITIAL), }
    #endif

p99_inline p99_futex *p99_futex_init (p99_futex *p00_fut, unsigned p00_ini) {
    if (p00_fut) {
...
p99_inline void p99_futex_destroy (p99_futex *p00_fut) {
    if (p00_fut) {
...
unsigned p99_futex_load (volatile p99_futex *p00_fut) {
    unsigned p00_ret = 0;
...
p99_inline unsigned p00_futex_wakeup (volatile p99_futex *p00_fut, unsigned p00_wmin, unsigned p00_wmax) {
    assert (p00_wmin <= p00_wmax);
    if (p00_wmax && p00_fut->p99_waiting) {
        if (p00_wmax > p00_fut->p99_waiting)
            p00_wmax = p00_fut->p99_waiting;
        if (p00_wmax > 1u)
            cnd_broadcast ((cnd_t *) &p00_fut->p99_cnd);
        else
            cnd_signal ((cnd_t *) &p00_fut->p99_cnd);
...
p99_inline void p00_futex_wait (volatile p99_futex *p00_fut) {
    ++p00_fut->p99_waiting;
...
unsigned p99_futex_add (
    ...
    P99_MUTUAL_EXCLUDE (*(mtx_t *) &p00_fut->p99_mut) {
        p00_ret                         = p00_fut->p99_cnt;
        register const unsigned p00_des = p00_ret + p00_hmuch;
    ...
    if (p00_wmin)
        p99_futex_wakeup (p00_fut, p00_wmin, p00_wmax);
    return p00_ret;
}
