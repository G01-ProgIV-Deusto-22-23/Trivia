...
P00_DOCUMENT_DECLARATION_ARGUMENT (P99_DEFINE_UNION, 3)
    #define P99_DEFINE_UNION(NAME, ...)                                                                                \
        /** \remark This union has a hidden first member that is just a byte array to ensure that the default          \
         * initializer <code>{ 0 }</code> will zero out the whole structure. To initialize a particular member, you    \
         * should always use a designated initializer such as <code>{ .toto = { .a = 1, }, }</code> **/                \
        union NAME {                                                                                                   \
...
#define P99_ENC_DECLARE(T, NAME)                                                                                       \
    /** @remark encapsulates an object of type T @see P99_ENC for access through object @see P99_ENCP for access       \
     * through pointer **/                                                                                             \
    struct NAME {                                                                                                      \
            /** @private */                                                                                            \
            T p00_val;                                                                                                 \
