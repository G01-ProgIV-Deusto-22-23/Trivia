...
#define P00_ERRNO_LIST                                                                                                 \
    2BIG, ACCES, ADDRINUSE, ADDRNOTAVAIL, AFNOSUPPORT, /* POSIX allows these two to be equal */                        \
        P99_IF_EQ (EAGAIN, EWOULDBLOCK) (AGAIN, ) (AGAIN, WOULDBLOCK, ) ALREADY, BADE, BADF, BADFD, BADMSG, BADR,      \
        BADRQC, BADSLT, BUSY, CANCELED, CHILD, CHRNG, COMM, CONNABORTED, CONNREFUSED,                                  \
        CONNRESET, /* POSIX has only EDEADLK, Linux also has EDEADLOCK */                                              \
        P99_IF_EQ (EDEADLK, EDEADLOCK) (DEADLK, ) (DEADLK, DEADLOCK, ) DESTADDRREQ, DOM, DQUOT, EXIST, FAULT, FBIG,    \
        HOSTDOWN, HOSTUNREACH, IDRM, ILSEQ, INPROGRESS, INTR, INVAL, IO, ISCONN, ISDIR, ISNAM, KEYEXPIRED,             \
        KEYREJECTED, KEYREVOKED, L2HLT, L2NSYNC, L3HLT, L3RST, LIBACC, LIBBAD, LIBEXEC, LIBMAX, LIBSCN, LOOP,          \
        MEDIUMTYPE, MFILE, MLINK, MSGSIZE, MULTIHOP, NAMETOOLONG, NETDOWN, NETRESET, NETUNREACH, NFILE, NOBUFS,        \
        NODATA, NODEV, NOENT, NOEXEC, NOKEY, NOLCK, NOLINK, NOMEDIUM, NOMEM, NOMSG, NONET, NOPKG, NOPROTOOPT, NOSPC,   \
        NOSR, NOSTR, NOSYS, NOTBLK, NOTCONN, NOTDIR, NOTEMPTY, NOTSOCK, NOTTY, NOTUNIQ,                                \
        NXIO, /* should be distinct for POSIX, but are equal on Linux */                                               \
        P99_IF_EQ (ENOTSUP, EOPNOTSUPP) (NOTSUP, ) (NOTSUP, OPNOTSUPP, ) /* is defined by SVID as such: OVERFLOW, */   \
        PERM,                                                                                                          \
        PFNOSUPPORT, PIPE, PROTO, PROTONOSUPPORT, PROTOTYPE, RANGE, REMCHG, REMOTE, REMOTEIO, RESTART, ROFS, SHUTDOWN, \
        SOCKTNOSUPPORT, SPIPE, SRCH, STALE, STRPIPE, TIME, TIMEDOUT, TXTBSY, UCLEAN, UNATCH, USERS, XDEV, XFULL, XXX
...
enum {
    ...
    P99_SEQ (P00_ERRNO_ENUM, P00_ERRNO_LIST),
    P00_ERRNO_ENUM (OVERFLOW),
    /**
...
#define P00_ERRNO_CASE_(ERR, STR)                                                                                      \
    case ERR:                                                                                                          \
        return STR
 ...
 **/
P99_CONST_FUNCTION
p99_inline const char *p99_errno_getname (errno_t p00_err) {
    switch (p00_err) {
