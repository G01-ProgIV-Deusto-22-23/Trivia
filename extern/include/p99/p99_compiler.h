...
/* be sure to put all compilers that are faking gcc before gcc itself */
    ...
    #define P99_COMPILER_VERSION                                                                                       \
        "Apple/clang " __apple_build_version__ "pretending clang version " __clang_version__                           \
        "; gnu " P99_STRINGIFY (__GNUC__) "." P99_STRINGIFY (__GNUC_MINOR__) "." P99_STRINGIFY (__GNUC_PATCHLEVEL__)
    ...
    #undef P99_COMPILER_VERSION
    #define P99_COMPILER_VERSION                                                                                       \
        "intel " P99_STRINGIFY (__INTEL_COMPILER) "; gnu " P99_STRINGIFY (__GNUC__) "." P99_STRINGIFY (__GNUC_MINOR__  \
    ...
    #undef P99_COMPILER_VERSION
    #define P99_COMPILER_VERSION                                                                                       \
        "pcc " P99_STRINGIFY (__PCC__) "." P99_STRINGIFY (__PCC_MINOR__) "." P99_STRINGIFY (__PCC_MINORMINOR__         \
        ) "; gnu " P99_STRINGIFY (__GNUC__) "." P99_STRINGIFY (__GNUC_MINOR__) "." P99_STRINGIFY (__GNUC_PATCHLEVEL__)
    #define P99_VERSION_NO P00_VERSION_NO (__PCC__, __PCC_MINOR__, __PCC_MINORMINOR__)

#elif P99_COMPILER & P99_COMPILER_TINYC
    #undef P99_COMPILER_VERSION
    #define P99_COMPILER_VERSION "tinyc " P99_STRINGIFY (__TINYC__)
...
enum {
    p00_trailing_comma_in_enum__ = -1,
};

inline signed p00_trailing_comma_in_initializer__ (void) {
    signed a [] = {
        p00_trailing_comma_in_enum__,
    };
    return a [0];
}
    #else
    ...
    #define p99_has_extension(X)                                                                                       \
        (__has_extension (X) || p00_has_extension_##X) // Compatibility with non-clang compilers.
    #define p99_has_attribute(X)                                                                                       \
        (__has_attribute (X) || p00_has_attribute_##X) // Compatibility with non-clang compilers.
#endif
 ...
 **/
     ...
     **/
        ...
        #define static_assert(EXPR, DIAGSTR)                                                                           \
            extern char const p00_compiletime_assert [sizeof ((void const *[3 *(!!(EXPR)) - 1]                         \
            ) { &p00_compiletime_assert, "static assertion failed: " P99_STRINGIFY (EXPR) ", " DIAGSTR })]
extern const char p00_compiletime_assert [sizeof (const void *[2])];
    #endif
#endif
...
#define P00_HARMLESS_SIZEOF(ID) sizeof (sizeof (ID))
    ...
    #define P00_UNUSED(EXPR)                                                                                           \
        extern char const p00_harmless_declaration [P00_HARMLESS_SIZEOF (EXPR)] __attribute__ ((__unused__))
#else
