...
#define P00_CASE_RETURN(NAME, X, I)                                                                                    \
    case I:                                                                                                            \
        return X
...
#define P99_CHOICE_FUNCTION(TYPE, NAME, DEFAULT, ...)                                                                  \
    ...
    P99_CONST_FUNCTION                                                                                                 \
    p99_inline TYPE NAME (size_t p00_x) {                                                                              \
        switch (p00_x) {                                                                                               \
...
P99_CHOICE_FUNCTION (
    uint8_t, p99_small_primes, 0, 2, 3, 5, 7, 11, 13, 17, 19, 23, 31, 37, 41, 43, 47, 53, 57, 59, 61, 67, 71
);
...
#define P00_UNIQUE_BIT_HASH(X, BITS, WIDTH)                                                                            \
    ((((X) *P00_UNIQUE_BIT_MULT (BITS)) >> (WIDTH - BITS)) & P99_PASTE3 (UINT, WIDTH, _MAX))

#define P00_UNIQUE_BIT_(BIT, BITS, WIDTH) P00_UNIQUE_BIT_HASH (P99_PASTE3 (UINT, WIDTH, _C) (1) << BIT, BITS, WIDTH)

#define P00_UNIQUE_BIT_RETURN(NAME, X, I)                                                                              \
    case P00_UNIQUE_BIT_ (I, X, NAME):                                                                                 \
        return I

#ifdef P00_DOXYGEN
    /* doxygen can't handle the P99_FOR */
    #define P00_UNIQUE_BIT_FUNCTION(TYPE, NAME, DEFAULT, BITS, WIDTH)                                                  \
        p99_inline TYPE P99_PASTE2 (NAME, BITS) (size_t p00_x)
#else
    #define P00_UNIQUE_BIT_FUNCTION(TYPE, NAME, DEFAULT, BITS, WIDTH)                                                  \
        P99_CONST_FUNCTION                                                                                             \
        p99_inline TYPE P99_PASTE2 (NAME, BITS) (P99_PASTE3 (uint, WIDTH, _t) p00_x) {                                 \
            switch (p00_x) {                                                                                           \
...
#define P00_UNIQUE_BIT(BITS, WIDTH)                                                                                    \
    ...
    P99_CONST_FUNCTION                                                                                                 \
    p99_inline unsigned P99_PASTE2 (p99_unique_bit_, WIDTH) (P99_PASTE3 (uint, WIDTH, _t) p00_x) {                     \
        ...
        return P99_PASTE2 (p00_unique_bit_hash_, BITS) (P00_UNIQUE_BIT_HASH (p00_x, BITS, WIDTH));                     \
    }                                                                                                                  \
    /*! @brief Find the one unique bit that is set in @a p00_x                 */                                      \
    /*! if @a p00_x has 0 or more than one bits set this returns a big number. */                                      \
    /*! @see p99_unique_bit_ ## WIDTH                                      */                                          \
    P99_CONST_FUNCTION                                                                                                 \
    p99_inline unsigned P99_PASTE2 (p99_unique_bit_checked_, WIDTH) (P99_PASTE3 (uint, WIDTH, _t) p00_x) {             \
