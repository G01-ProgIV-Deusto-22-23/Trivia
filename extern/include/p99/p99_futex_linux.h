...
p99_inline int p00_futex (
    int                   *uaddr,   /*!< the base address to be used */
 ...
 ** @see p00_futex_broadcast
 **/
p99_inline int p00_futex_wait_once (int *uaddr, int val) {
 ...
 ** @see p00_futex_broadcast
 **/
p99_inline int p00_futex_wake (int *uaddr, int wakeup) {
 ...
 ** @see p00_futex_broadcast
 **/
p99_inline int p00_futex_signal (int *uaddr) {
 ...
 ** @see p00_futex_signal
 **/
p99_inline int p00_futex_broadcast (int *uaddr) {
...
p99_inline unsigned p99_futex_load (volatile p99_futex *p00_fut) {
    return atomic_load (p00_fut);
}

p99_inline void p99_futex_wakeup (volatile p99_futex *p00_cntp, unsigned p00_wmin, unsigned p00_wmax) {
    if (p00_wmax < p00_wmin)
        p00_wmax = p00_wmin;
    if (p00_wmax) {
        volatile unsigned *const p00_cnt = (unsigned *) p00_cntp;
        static_assert (sizeof *p00_cntp == sizeof *p00_cnt, "linux futex supposes that there is no hidden lock field");
        for (;;) {
...
p99_inline void p99_futex_wait (volatile p99_futex *p00_cntp) {
    volatile unsigned *const p00_cnt = (unsigned *) p00_cntp;
    static_assert (sizeof *p00_cntp == sizeof *p00_cnt, "linux futex supposes that there is no hidden lock field");
    for (;;) {
...
p99_inline unsigned p99_futex_add (
    volatile p99_futex *futex, unsigned p00_hmuch, unsigned p00_cstart, unsigned p00_clen, unsigned p00_wmin,
    unsigned p00_wmax
) {
    unsigned                p00_act = atomic_fetch_add (futex, p00_hmuch);
    register const unsigned ret     = p00_act + p00_hmuch;
    if (p00_clen && P99_IN_RANGE (ret, p00_cstart, p00_clen))
        p99_futex_wakeup (futex, p00_wmin, p00_wmax);
    return p00_act;
}

p99_inline unsigned p99_futex_exchange (
    volatile p99_futex *futex, unsigned p00_desired, unsigned p00_cstart, unsigned p00_clen, unsigned p00_wmin,
    unsigned p00_wmax
) {
...
P00_DOCUMENT_IDENTIFIER_ARGUMENT (P99_FUTEX_COMPARE_EXCHANGE, 1)
            #define P99_FUTEX_COMPARE_EXCHANGE(FUTEX, ACT, EXPECTED, DESIRED, WAKEMIN, WAKEMAX)                        \
                do {                                                                                                   \
                    ...
                    static_assert (                                                                                    \
                        sizeof *p00Mcntp == sizeof *p00Mcnt,                                                           \
